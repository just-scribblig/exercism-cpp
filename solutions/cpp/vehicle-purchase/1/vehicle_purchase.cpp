...
std::string choose_vehicle(std::string option1, std::string option2) {
    ...
    return (option1 < option2)? (option1 + temp): (option2 + temp);
}
...
double calculate_resell_price(double original_price, double age) {
    ...
    if (age<3) return 0.8*original_price;
    else if (age<10) return 0.7*original_price;
    else return 0.5*original_price;
}
